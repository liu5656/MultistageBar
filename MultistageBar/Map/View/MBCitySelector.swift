...
class MBCitySelector: UIView {
    ...
    static func mb_show(callback: @escaping (String, String, String) -> Void) {
        let temp = MBCitySelector.init(frame: Screen.bounds)
        temp.picker.reloadAllComponents()
        temp.completion = callback
        UIApplication.shared.windows.first?.addSubview(temp)
//        UIApplication.shared.delegate?.window??.addSubview(temp)
    }
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let point = touches.first?.location(in: self), bgV.frame.contains(point) == false {
            mb_dismiss()
        }
    }
    func mb_dismiss() {
    ...
    @objc func mb_done() {
        let province = provinces[selectedProvince].name
        let city = provinces[selectedCity].cities[selectedCity].name
        let county = provinces[selectedProvince].cities[selectedCity].counties[selectedCounty]
        completion?(province, city, county)
        mb_dismiss()
    }
    
    private var selectedProvince = 0
    private var selectedCity = 0
    private var selectedCounty = 0
    var completion: ((String, String, String) -> Void)?
    private lazy var bgV: UIView = {
        let temp = UIView.init(frame: CGRect.init(x: 0, y: bounds.height - 300, width: bounds.width, height: 300))
        temp.backgroundColor = UIColor.white
        addSubview(temp)
        return temp
    }()
    private lazy var doneB: UIButton = {
        let but = UIButton.init(type: .custom)
        but.frame = CGRect.init(x: bgV.frame.width - 80, y: 0, width: 60, height: 30)
        but.titleLabel?.font = UIFont.systemFont(ofSize: 14, weight: .regular)
        but.setTitleColor(UIColor.black, for: .normal)
        but.setTitle("完成", for: .normal)
        but.addTarget(self, action: #selector(mb_done), for: .touchUpInside)
        bgV.addSubview(but)
        return but
    }()
    private lazy var picker: UIPickerView = {
        let temp = UIPickerView.init(frame: CGRect.init(x: 0, y: 0, width: bgV.frame.width, height: 300))
        temp.delegate = self
    ...
    private lazy var provinces: [MBProvince] = {
        guard let data = datas.data(using: .utf8),
              let provincesInfo = try? JSONSerialization.jsonObject(with: data, options: .allowFragments) as? [String: Any] else {
            return []
        }
        let provinces = provincesInfo.keys.map { (provinceKey) -> MBProvince in
            var province = MBProvince.init()
            province.name = provinceKey as String
            if let citysInfo = provincesInfo[provinceKey as String] as? [String: Any] {
                province.cities = citysInfo.keys.map { (cityKey) -> MBCity in
                    let city = MBCity.init()
                    city.name = cityKey as String
                    city.counties = citysInfo[city.name] as! [String]
                    return city
                }
            }
            return province
...
extension MBCitySelector: UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        ...
        case 1:
            return provinces[selectedProvince].cities[row].name
        case 2:
            return provinces[selectedProvince].cities[selectedCity].counties[row]
        default:
            return nil
        }
    }
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        ...
        case 1:
            return provinces[selectedProvince].cities.count
        case 2:
            return provinces[selectedProvince].cities[selectedCity].counties.count
        default:
            return 0
        }
    }
